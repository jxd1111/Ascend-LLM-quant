...
## Validated W8A8 profile

Current registrations: `("W8A8_MIX", "linear")` and
`("W8A8_MIX", "moe")`.

The offline recipe selects the activation-quantization algorithm that produced
the best measured result for the validated model. That algorithm remains an
implementation detail; the exported artifact and plugin capability are named
W8A8.
...
This runtime branch is a **deployment-role decision**, not a token-by-token
prefill/decode branch. A normal non-disaggregated server follows the dynamic
path for both phases. The plugin exports one W8A8 capability and uses a separate
explicit `execution_role` capability to select its runtime path.
